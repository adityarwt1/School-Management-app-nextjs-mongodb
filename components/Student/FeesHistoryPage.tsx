  ...
  return (
    <div className="border mx-2 border-black/15 rounded-xl w-[30%] px-2  gap-2 flex flex-col py-5">
      <div className="w-full px-4 py-1 flex justify-evenly">
        ...
        <div className="w-full">Remaining</div>
      ...
      {feesData ? (
        feesData.map((ele) => (
          <>
            ...
            >
              <div className="text-left flex w-full">{Months[ele.month]}</div>
              <div className="text-green-500 w-full">
                <span className="font-mono">{`₹${ele.amount}`}</span>
              </div>
              <div className="text-yellow-600 w-full font-mono">{`₹${ele.remains}`}</div>
              {ele.remains > 0 ?  (
                <Link href={`/payFees?mode=payRemains&_id=${ele._id}&amount=${ele.remains}&month=${ele.month}`}>
                ...
                <button className='bg-green-600 text-white px-2 py-1 rounded-md  hover:opacity-95 ' aria-disabled={true}>Paid</button>
              )}
            </div>
          </>
        ))
