import {
  ...
  PrincipleLoginResponse,
} from "@/interfaces/Principle/Register/Login/PrincipleRequestAndResponse";
...
export async function POST(
  req: NextRequest
): Promise<NextResponse<PrincipleLoginResponse>> {
  try {
    const { email, password }: PrincipleLoginRequest = await req.json();

    if (!email || !password) {
      return NextResponse.json(
        { success: false, error: "field not provided!" },
        { status: 400 }
      );
    }
    ...
    const principle = await Principle.findOne({
      email,
    }).lean<PrincipleInfoInterface>();

    if (!principle) {
      return NextResponse.json(
        { success: false, error: "Principle Record not found!" },
        { status: 404 }
      );
    }
    ...
    if (!isPassowrdTrue) {
      return NextResponse.json(
        { error: "Wrong password!", success: false },
        { status: 400 }
      );
    }
